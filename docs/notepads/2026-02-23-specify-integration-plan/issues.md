...
- Potential behavior change: Projects without `.opencode/opencode.json{,c}` _and_ without `.opencode/ghostwire.json{,c}` will no longer receive Ghostwire builtin agents via config composition.
