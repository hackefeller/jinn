 ...
 * Phind adapter
 *
 * Formats commands and skills for Phind.
 * Phind uses:
 * - Skills: .phind/skills/<name>/SKILL.md
 * - Commands: .phind/commands/<name>.md
...
export const phindAdapter: ToolCommandAdapter = {
  toolId: 'phind',
  toolName: 'Phind',
  skillsDir: '.phind',

  getCommandPath(commandId: string): string {
    return path.join('.phind', 'commands', `ghostwire-${commandId}.md`);
  },

  getSkillPath(skillName: string): string {
    return path.join('.phind', 'skills', skillName, 'SKILL.md');
  },
