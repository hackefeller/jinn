 ...
 * LM Studio adapter
 *
 * Formats commands and skills for LM Studio.
 * LM Studio uses:
 * - Skills: .lmstudio/skills/<name>/SKILL.md
 * - Commands: .lmstudio/commands/<name>.md
...
export const lmStudioAdapter: ToolCommandAdapter = {
  toolId: 'lm-studio',
  toolName: 'LM Studio',
  skillsDir: '.lmstudio',

  getCommandPath(commandId: string): string {
    return path.join('.lmstudio', 'commands', `ghostwire-${commandId}.md`);
  },

  getSkillPath(skillName: string): string {
    return path.join('.lmstudio', 'skills', skillName, 'SKILL.md');
  },
