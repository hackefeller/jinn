 ...
 * Tabby adapter
 *
 * Formats commands and skills for Tabby.
 * Tabby uses:
 * - Skills: .tabby/skills/<name>/SKILL.md
 * - Commands: .tabby/commands/<name>.md
...
export const tabbyAdapter: ToolCommandAdapter = {
  toolId: 'tabby',
  toolName: 'Tabby',
  skillsDir: '.tabby',

  getCommandPath(commandId: string): string {
    return path.join('.tabby', 'commands', `ghostwire-${commandId}.md`);
  },

  getSkillPath(skillName: string): string {
    return path.join('.tabby', 'skills', skillName, 'SKILL.md');
  },
