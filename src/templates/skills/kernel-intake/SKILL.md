...
description: Intake, classify, and correctly place new bugs, ad-hoc requests,
  and unplanned work into Linear. Resolves team, workflow status, labels,
  relation type, and optionally cycle and milestone before creating the issue.
  Use when a new bug report arrives, an unstructured request needs placement, or
  work needs to be positioned correctly in the Linear hierarchy.
license: MIT
compatibility: Requires Linear access for issue, project, team, label, and
  workflow status reads and writes.
metadata:
  author: project
  version: "1.0"
  category: Workflow
...
termination:
  - Linear issue created with correct team, status, parent, priority, labels,
    and relations
