...
export function getOpenspecProposeSkillTemplate(): SkillTemplate {
  return {
    ...
    license: 'MIT',
    compatibility: 'Requires ghostwire CLI.',
    metadata: {
      author: 'ghostwire',
      version: '1.0',
      category: 'Workflow',
...
2. **Create the change directory**
   \`\`\`bash
   ghostwire new change "<name>"
   \`\`\`
   This creates a scaffolded change at \`ghostwire/changes/<name>/\`.
...
- Prompt: "Run \`/ghostwire:apply\` or ask me to implement to start working on the tasks."
...
export function getOpenspecApplySkillTemplate(): SkillTemplate {
  return {
    name: 'ghostwire-apply',
    description: 'Implement tasks from a Ghostwire change. Use when user wants to execute the implementation plan.',
    license: 'MIT',
    compatibility: 'Requires ghostwire CLI.',
    metadata: {
      author: 'ghostwire',
    ...
    instructions: `Implement tasks from a Ghostwire change.
...
1. **Select the change**
   ...
   - If ambiguous, run \`ghostwire list\` to get available changes
...
export function getOpenspecArchiveSkillTemplate(): SkillTemplate {
  return {
