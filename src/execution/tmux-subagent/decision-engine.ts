...
export function calculateCapacity(windowWidth: number, windowHeight: number): GridCapacity {
  const availableWidth = Math.floor(windowWidth * (1 - MAIN_PANE_RATIO));
  const cols = Math.min(
    MAX_GRID_SIZE,
    Math.max(0, Math.floor((availableWidth + DIVIDER_SIZE) / (MIN_PANE_WIDTH + DIVIDER_SIZE))),
  );
  const rows = Math.min(
    MAX_GRID_SIZE,
    Math.max(0, Math.floor((windowHeight + DIVIDER_SIZE) / (MIN_PANE_HEIGHT + DIVIDER_SIZE))),
  );
...
export function mapPaneToSlot(pane: TmuxPaneInfo, plan: GridPlan, mainPaneWidth: number): GridSlot {
  const rightAreaX = mainPaneWidth;
  const relativeX = Math.max(0, pane.left - rightAreaX);
  const relativeY = pane.top;

  const col =
    plan.slotWidth > 0 ? Math.min(plan.cols - 1, Math.floor(relativeX / plan.slotWidth)) : 0;
  const row =
    plan.slotHeight > 0 ? Math.min(plan.rows - 1, Math.floor(relativeY / plan.slotHeight)) : 0;
...
function findFirstEmptySlot(occupancy: Map<string, TmuxPaneInfo>, plan: GridPlan): GridSlot {
