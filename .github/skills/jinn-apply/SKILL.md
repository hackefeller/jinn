...
Implement work from Linear using Linear MCP. Work on exactly one issue at a time. Every state change in your work must be reflected back into the issue immediately.
...
- Identify the target scope: a project ID, parent issue ID, or a single issue ID provided by the user.
- Use `mcp_linear_list_issues` (filtered to scope + `state: "Todo"`) to confirm the full queue is visible before starting.
- Read the first issue with `mcp_linear_get_issue` (+ `includeRelations: true`) to confirm it has no `blockedBy` dependencies. If it does, surface the blocker and stop.

### 1. Select the next issue

Use this priority order to find the next issue:
...
- If it has `blocks` relations, those issues are now unblocked — pick the highest-priority one among them as the next issue.
- This is the canonical "next" when an explicit dependency chain was modelled.

**b. Sibling sub-issues (implicit ordering)**

- If the completed issue has a `parentId`, call `mcp_linear_list_issues` with `parentId` set to that parent and `state: "Todo"`.
- Pick the highest-priority unstarted sibling.

**c. Project backlog fallback**

- If neither applies, call `mcp_linear_list_issues` with the target `project` and `state: "Todo"`, ordered by priority.
- Pick the top unblocked issue.

After identifying the candidate, call `mcp_linear_get_issue` (with `includeRelations: true`) to confirm it has no unresolved `blockedBy` relations before claiming it.
...
- If a blocker surfaces mid-implementation, stop, add a comment explaining the blocker, transition the issue to **Blocked**, and do not proceed to the next issue.

### 4. Update the issue on completion

After implementation passes verification:

- Use `mcp_linear_save_comment` to leave a concise summary comment:
...
- Pause on ambiguity or blockers instead of guessing — surface the question in a Linear comment.
- Sub-issues are the execution queue; do not skip or reorder unless explicitly instructed.
